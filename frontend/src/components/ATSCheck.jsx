...
import { Upload, FileText, Search, AlertCircle, CheckCircle, XCircle, TrendingUp, BarChart3 } from 'lucide-react';
...
const ATSCheck = () => {
  ...
  const fetchJobs = async () => {
    try {
      const res = await axios.get('http://localhost:3001/api/v1/job/get', {
            ...
            <div className="border-t border-gray-200 dark:border-[#2a2a2a] p-8 space-y-8">
              ...
              <div>
                ...
                <div className="relative h-4 bg-gray-200 dark:bg-[#2a2a2a] rounded-full overflow-hidden">
                  <div
                    className={`h-full ${getScoreColor(result.ats_score)} transition-all duration-1000 ease-out`}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="p-4 rounded-lg bg-purple-50 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-900">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-sm font-medium text-purple-900 dark:text-purple-300">Skill Match</span>
                    </div>
                    <p className="text-2xl font-bold text-purple-700 dark:text-purple-300">{result.details.skill_match_percentage}%</p>
                  </div>
                  <div className="p-4 rounded-lg bg-cyan-50 dark:bg-cyan-950/20 border border-cyan-200 dark:border-cyan-900">
                    <div className="flex items-center gap-2 mb-2">
                      <BarChart3 className="h-4 w-4 text-cyan-600 dark:text-cyan-400" />
                      <span className="text-sm font-medium text-cyan-900 dark:text-cyan-300">Keyword Match</span>
                    </div>
                    <p className="text-2xl font-bold text-cyan-700 dark:text-cyan-300">{result.details.keyword_match_percentage}%</p>
                  </div>
                  <div className="p-4 rounded-lg bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-200 dark:border-indigo-900">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-sm font-medium text-indigo-900 dark:text-indigo-300">Section Completeness</span>
                    </div>
                    <p className="text-2xl font-bold text-indigo-700 dark:text-indigo-300">{result.details.section_completeness}%</p>
                  </div>
                </div>
              )}
              ...
              {result.resume_sections_to_improve && result.resume_sections_to_improve.length > 0 && (
                <div>
                  <h4 className="font-semibold text-gray-900 dark:text-white mb-3">
                    Missing Resume Sections
                  </h4>
                  <div className="space-y-2">
                    {result.resume_sections_to_improve.map((section, idx) => (
                      <div
                        key={idx}
              ...
              {result.details && (
                <div className="p-4 rounded-lg bg-gray-50 dark:bg-[#121212] border border-gray-200 dark:border-[#2a2a2a]">
                  <p className="text-sm text-gray-600 dark:text-[#808080]">
                    Found <strong className="text-gray-900 dark:text-white">{result.details.total_resume_skills}</strong> skills in your resume • 
                    Job requires <strong className="text-gray-900 dark:text-white">{result.details.total_job_skills}</strong> skills • 
                    You match <strong className="text-green-600 dark:text-green-400">{result.matched_skills?.length || 0}</strong> skills
                  </p>
                </div>
              )}
            </div>
